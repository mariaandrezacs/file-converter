"""HTTP routes for CSV to JSON conversion."""

from typing import Annotated

from fastapi import APIRouter, Depends, File, HTTPException, UploadFile

from src.csv_to_json.services.conversion_service import ConversionService
...
async def convert_csv_to_json(
    ...
    """Handle CSV to JSON conversion request."""
    if not file.filename.lower().endswith(".csv"):
        raise HTTPException(status_code=400, detail="Arquivo deve ser CSV")

    try:
        return service.convert_csv_to_json(
            file.file,
            options={
                "delimiter": delimiter,
                "encoding": encoding,
